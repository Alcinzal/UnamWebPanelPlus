...
if(getParam('csrf_token') != $_SESSION['csrf_token']) {
    ...
    if (!empty($loginBlock) && ($loginBlock['lb_logintries'] >= $config['failedlogin_blocktries'] || $loginBlock['lb_logintries'] == 0) && (time() - strtotime($loginBlock['lb_datetime'])) / 60 < $loginBlock['lb_blocktime']) {
        $base->unam_echoFailure(sprintf($larr['login_blocked'], ($loginBlock['lb_blocktime'] - (int)((time() - strtotime($loginBlock['lb_datetime'])) / 60))));
        ...
        if (getParam('password') === $config['password']) {
            ...
            if (empty($loginBlock) || ((time() - strtotime($loginBlock['lb_datetime'])) / 60 >= $loginBlock['lb_blocktime'])) {
                $base->unam_dbDelete(getConn(), "loginblock", ['lb_ip' => $hostAddress]);
